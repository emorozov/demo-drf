...
class FeedDateFilter(FilterSet):
    from_datetime = DateTimeFilter(
        field_name="feeds__created_on", lookup_expr="gte", distinct=True
    )
    to_datetime = DateTimeFilter(
        field_name="feeds__created_on", lookup_expr="lte", distinct=True
    )
